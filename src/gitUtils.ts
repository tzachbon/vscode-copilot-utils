...
export class GitUtils {
    ...
    public static async getBranchChanges(sourceBranch?: string, targetBranch?: string): Promise<GitChange[]> {
        const gitResult = await this.getGit();
        if (!gitResult) {
            throw new Error('Failed to initialize Git');
        }
    ...
    public static async formatBranchChanges(): Promise<string> {
        try {
            ...
            if (reachedLimit) {
                ...
                result += `\n... and ${totalChanges - totalLines} more changes not shown (limit: ${maxLineCount} lines)`;
            }
            
            return result.trim();
        } catch (error) {
            console.error('Error formatting branch changes:', error);
            return `Error getting branch changes: ${error instanceof Error ? error.message : String(error)}`;
        }
    }
}
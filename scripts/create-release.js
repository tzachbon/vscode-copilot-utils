...
if (!['patch', 'minor', 'major'].includes(versionType)) {
  console.error(
    `${colors.red}Error: Version type must be one of 'patch', 'minor', or 'major'${colors.reset}`,
  );
  process.exit(1);
}
...
try {
  ...
  if (status.trim()) {
    console.error(
      `${colors.red}Error: There are uncommitted changes in the repository.${colors.reset}`,
    );
    console.error(
      `${colors.yellow}Please commit or stash your changes before creating a release.${colors.reset}`,
    );
    process.exit(1);
  }
} catch (error) {
...
try {
  ...
  console.log(
    `${colors.yellow}Creating release for ${packageJson.displayName || packageJson.name}${
      colors.reset
    }`,
  );
  console.log(`${colors.blue}================================================${colors.reset}\n`);
  ...
  executeStep(
    ...
    `Committing version bump to ${newVersion}`,
  );
  ...
  if (shouldPush) {
    ...
    console.log(
      `${colors.green}✅ Successfully created and pushed release ${tagName}!${colors.reset}`,
    );
    console.log(
      `${colors.yellow}The GitHub Actions workflow should now be triggered to create a release.${colors.reset}`,
    );
  } else {
    console.log(
      `${colors.green}✅ Successfully created release ${tagName} locally!${colors.reset}`,
    );
    console.log(
      `${colors.yellow}To trigger the GitHub release workflow, push the tag with:${colors.reset}`,
    );
